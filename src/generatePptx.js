// CareTrack PowerPoint Export — runs in browser via CDN PptxGenJS
// This function is injected into App.jsx

/*
USAGE: Call generatePptx(entries, summary) from a button click.
entries = array of session objects from Supabase
...
export async function generatePptx(entries, summary = "") {
  ...
  await new Promise((resolve, reject) => {
    ...
    script.src = "https://cdn.jsdelivr.net/npm/pptxgenjs@3.12.0/libs/pptxgen.bundle.js";
  ...
  s1.addText(`${entries.length} sessions across ${hospitals.length || 1} hospital${hospitals.length !== 1 ? "s" : ""}`, { x: 0.55, y: 3.5, w: 6.0, h: 0.3, fontSize: 12, fontFace: "Calibri", color: "A8C8D0", margin: 0 });

  // HoverTech branding in right panel
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // SLIDE 2 — COMPLIANCE SUMMARY (7 metric cards)
  // ══════════════════════════════════════════════════════════════════════════
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // SLIDE 3 — TREND CHART (bar chart per metric)
  // ══════════════════════════════════════════════════════════════════════════
  ...
  const barData = [{
    name: "Avg Compliance %",
    labels: avgMetrics.map(m => m.label),
    values: avgMetrics.map(m => m.avg ?? 0),
  }];

  s3.addChart(pres.charts.BAR, barData, {
    x: 0.38, y: 1.55, w: 9.3, h: 3.7,
    barDir: "col",
    chartColors: avgMetrics.map(m => pctColor(m.avg)),
    chartArea: { fill: { color: BRAND.white }, roundedCorners: false },
    plotArea: { fill: { color: BRAND.white } },
    catAxisLabelColor: BRAND.inkLight,
    valAxisLabelColor: BRAND.inkLight,
    catAxisLabelFontSize: 9,
    valAxisLabelFontSize: 9,
    valAxisMaxVal: 100,
    valAxisMinVal: 0,
    valGridLine: { color: "E8E4E0", size: 0.5 },
  ...
  s3.addShape(pres.shapes.RECTANGLE, { x: 0.38, y: 5.3, w: 0.14, h: 0.14, fill: { color: BRAND.green }, line: { color: BRAND.green } });
  s3.addText("≥90% Target", { x: 0.58, y: 5.28, w: 2, h: 0.18, fontSize: 9, fontFace: "Calibri", color: BRAND.inkLight, margin: 0 });
  s3.addShape(pres.shapes.RECTANGLE, { x: 2.7, y: 5.3, w: 0.14, h: 0.14, fill: { color: BRAND.amber }, line: { color: BRAND.amber } });
  s3.addText("70-89% Monitor", { x: 2.9, y: 5.28, w: 2, h: 0.18, fontSize: 9, fontFace: "Calibri", color: BRAND.inkLight, margin: 0 });
  s3.addShape(pres.shapes.RECTANGLE, { x: 5.1, y: 5.3, w: 0.14, h: 0.14, fill: { color: BRAND.red }, line: { color: BRAND.red } });
  s3.addText("<70% Needs Attention", { x: 5.3, y: 5.28, w: 2.5, h: 0.18, fontSize: 9, fontFace: "Calibri", color: BRAND.inkLight, margin: 0 });

  // ══════════════════════════════════════════════════════════════════════════
  // SLIDE 4 — HOSPITAL COMPARISON (if multiple hospitals)
  // ══════════════════════════════════════════════════════════════════════════
  if (hospitals.length > 1) {
    ...
    const hBarData = [{
      name: "Overall Avg Compliance %",
      labels: hospitalAvgs.map(h => h.hospital),
      values: hospitalAvgs.map(h => h.overall ?? 0),
    }];

    s4.addChart(pres.charts.BAR, hBarData, {
      x: 0.38, y: 1.3, w: 9.3, h: 3.8,
      barDir: "col",
    ...
    // Session count labels
    s4.addText(`Sessions per hospital: ${hospitalAvgs.map(h => `${h.hospital}: ${h.sessions}`).join("  ·  ")}`, {
      x: 0.38, y: 5.2, w: 9.3, h: 0.28, fontSize: 10, fontFace: "Calibri", color: BRAND.inkLight, margin: 0
    });
  }
  ...
  const tableHeader = [
    { text: "Date",     options: { fill: { color: BRAND.primary }, color: BRAND.white, bold: true, fontSize: 9, fontFace: "Calibri", align: "center" } },
    { text: "Hospital", options: { fill: { color: BRAND.primary }, color: BRAND.white, bold: true, fontSize: 9, fontFace: "Calibri", align: "center" } },
    { text: "Location", options: { fill: { color: BRAND.primary }, color: BRAND.white, bold: true, fontSize: 9, fontFace: "Calibri", align: "center" } },
    { text: "MATT Appl.", options: { fill: { color: BRAND.primary }, color: BRAND.white, bold: true, fontSize: 9, fontFace: "Calibri", align: "center" } },
    { text: "Wedges",    options: { fill: { color: BRAND.primary }, color: BRAND.white, bold: true, fontSize: 9, fontFace: "Calibri", align: "center" } },
    { text: "Turning",   options: { fill: { color: BRAND.primary }, color: BRAND.white, bold: true, fontSize: 9, fontFace: "Calibri", align: "center" } },
    { text: "MATT Prop.", options: { fill: { color: BRAND.primary }, color: BRAND.white, bold: true, fontSize: 9, fontFace: "Calibri", align: "center" } },
    { text: "Wdg Room",  options: { fill: { color: BRAND.primary }, color: BRAND.white, bold: true, fontSize: 9, fontFace: "Calibri", align: "center" } },
    { text: "Offload",   options: { fill: { color: BRAND.primary }, color: BRAND.white, bold: true, fontSize: 9, fontFace: "Calibri", align: "center" } },
    { text: "Air Sup.",  options: { fill: { color: BRAND.primary }, color: BRAND.white, bold: true, fontSize: 9, fontFace: "Calibri", align: "center" } },
    { text: "Logged By", options: { fill: { color: BRAND.primary }, color: BRAND.white, bold: true, fontSize: 9, fontFace: "Calibri", align: "center" } },
  ];

  const tableRows = recentSessions.map((e, idx) => {
    const isEven = idx % 2 === 0;
    const rowBg = isEven ? BRAND.white : "F0EDEA";
    const cellOpts = (val, metricId) => {
      const p = metricId ? pct(e[`${metricId}_num`], e[`${metricId}_den`]) : null;
      const col = p !== null ? pctColor(p) : BRAND.ink;
      return { text: p !== null ? `${p}%` : val || "—", options: { fill: { color: rowBg }, color: metricId ? col : BRAND.ink, fontSize: 9, fontFace: "Calibri", align: "center", bold: metricId && p !== null } };
    };
    return [
      ...
      cellOpts(null, "matt_applied"),
      cellOpts(null, "wedges_applied"),
      cellOpts(null, "turning_criteria"),
      cellOpts(null, "matt_proper"),
      cellOpts(null, "wedges_in_room"),
      cellOpts(null, "wedge_offload"),
      cellOpts(null, "air_supply"),
  ...
  s5.addTable([tableHeader, ...tableRows], {
    ...
    colW: [0.75, 1.1, 0.9, 0.75, 0.75, 0.75, 0.85, 0.75, 0.75, 0.75, 1.1],
  });
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // SLIDE 6 — AI CLINICAL SUMMARY (if available)
  // ══════════════════════════════════════════════════════════════════════════
  if (summary && summary.length > 10) {
    ...
    // Wrap summary text
    const summaryText = summary.slice(0, 900);
    s6.addText(summaryText, { x: 0.75, y: 1.82, w: 8.7, h: 3.0, fontSize: 11, fontFace: "Calibri", color: BRAND.inkLight, lineSpacingMultiple: 1.4, valign: "top", margin: 0 });
