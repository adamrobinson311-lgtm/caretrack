...
const C = {
  bg: "#f7f5f0", surface: "#ffffff", surfaceAlt: "#f0ede6", border: "#e2ddd4", borderDark: "#ccc6b8",
  ink: "#1a1814", inkMid: "#5a5448", inkLight: "#9c9488", inkFaint: "#c8c2b4",
  green: "#2d7a4f", greenLight: "#e8f5ee", greenMid: "#5aaa78",
  red: "#c0392b", redLight: "#fdf0ee", amber: "#d97706", amberLight: "#fef3c7",
  ...
  <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
    ...
    {["All", ...hospitals].map(h => (
      <button key={h} onClick={() => onChange(h)}
        style={{ padding: "4px 12px", borderRadius: 20, fontSize: 11, fontFamily: "'IBM Plex Mono', monospace", cursor: "pointer", transition: "all 0.15s", border: `1px solid ${value === h ? C.ink : C.border}`, background: value === h ? C.ink : "none", color: value === h ? C.surface : C.inkMid, letterSpacing: "0.05em" }}>
        {h}
      </button>
    ))}
  </div>
...
export default function App() {
  ...
  useEffect(() => {
    const fetchSessions = async () => {
      ...
      if (error) { setDbError("Could not connect to database. Check your Supabase credentials."); }
  ...
  const handleSave = async () => {
    ...
    if (error) { setSaveError("Failed to save. Check your connection and try again."); setSaving(false); return; }
  ...
  const Tab = ({ id, label }) => (
    <button onClick={() => setTab(id)} style={{ padding: "10px 22px", background: "none", border: "none", cursor: "pointer", fontFamily: "'IBM Plex Mono', monospace", fontSize: 11, letterSpacing: "0.08em", color: tab === id ? C.ink : C.inkLight, borderBottom: tab === id ? `2px solid ${C.ink}` : "2px solid transparent", transition: "all 0.15s" }}>{label}</button>
  );
  ...
  return (
    <div style={{ background: C.bg, minHeight: "100vh", color: C.ink, fontFamily: "'IBM Plex Sans', sans-serif" }}>
      ...
      {dbError && <div style={{ background: C.redLight, borderBottom: `1px solid #f5c6c2`, padding: "12px 32px" }}><div style={{ maxWidth: 1080, margin: "0 auto", fontSize: 13, color: C.red }}>⚠ {dbError}</div></div>}
      ...
      <div style={{ maxWidth: 1080, margin: "0 auto", padding: "32px 32px 64px" }}>
        ...
        {tab === "log" && (
          <div style={{ maxWidth: 720 }}>
            ...
            {saveError && <div style={{ marginBottom: 16, padding: "10px 14px", background: C.redLight, border: `1px solid #f5c6c2`, borderRadius: 8, fontSize: 13, color: C.red }}>⚠ {saveError}</div>}
          ...
          <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 28, flexWrap: "wrap", gap: 12 }}>
              <div>
                ...
                <p style={{ color: C.inkMid, fontSize: 13 }}>{loading ? "Loading sessions..." : `Average compliance across ${filteredDashboard.length} session${filteredDashboard.length !== 1 ? "s" : ""}${hospitalFilter !== "All" ? ` · ${hospitalFilter}` : ""}.`}</p>
              </div>
            ...
            {loading ? (
              <div style={{ padding: "60px 0", textAlign: "center", color: C.inkLight, fontFamily: "'IBM Plex Mono', monospace", fontSize: 12 }}>Loading data from database...</div>
            ) : (
              <>
                    ...
                    <div key={e.id} style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, padding: "18px 20px" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
                        <div>
                          ...
                          {e.hospital && <div style={{ fontSize: 13, color: C.ink, marginTop: 2, fontWeight: 500 }}>{e.hospital}</div>}
