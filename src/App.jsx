...
export default function App() {
  ...
  const handleCreateInvite = async () => {
    ...
    try {
      // Insert invite row — token + expires_at are filled by DB defaults
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // Admins get all sessions + audit log + user profiles + clinical invites + hospital list
      if (isAdminUser) {
        ...
        {tab === "admin" && isAdmin && (
          <div>
            ...
            <div style={{ display: "flex", borderBottom: `1px solid ${C.border}`, marginBottom: 24, gap: 0, overflowX: "auto", WebkitOverflowScrolling: "touch" }}>
              {[["sessions","ALL SESSIONS"],["users","USER MANAGEMENT"],["invites","CLINICAL INVITES"],["audit","AUDIT LOG"]].map(([id, label]) => (
