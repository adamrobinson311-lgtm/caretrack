...
const createEmptyBed = (metrics, roomNum) => {
  ...
  metrics.forEach(m => { bed[`${m.id}_q`] = "0"; bed[`${m.id}_a`] = "0"; bed[`${m.id}_na`] = false; bed[`${m.id}_touched`] = false; });
  return bed;
};
      ...
      <div style={{ background: C.surface, border: `1px solid ${isNa ? C.border : C.primary + "44"}`, borderRadius: 14, overflow: "hidden", opacity: isNa ? 0.6 : 1, transition: "opacity 0.2s" }}>
        ...
        <div style={{ padding: "8px 16px 16px" }}>
          {metrics.map((m, mi) => {
            ...
            // Default to NO (compliant === false). _touched is a separate flag that
            // records whether the user has actively tapped — used by the progress bar
            // and auto-sum to distinguish "intentionally NO" from "untouched default".
            const touched = !!bed[`${m.id}_touched`];
...
export default function App() {
  ...
  // ── Clinical invite helpers ─────────────────────────────────────────────
  const buildInviteUrl = (token) => `${window.location.origin}/invite/${token}`;
  ...
  const handleResendInvite = async (invite) => {
    ...
    try {
      ...
      if (fnError) throw fnError;
      await logAudit("CLINICAL_INVITE_RESENT", { hospital: invite.hospital, email: invite.email });
      setInviteSuccess(`Invite resent to ${invite.email}`);
      setTimeout(() => setInviteSuccess(null), 4000);
    } catch (e) {
      setInviteError("Failed to resend: " + (e.message || "unknown error"));
    ...
    (async () => {
      ...
      if (userHospitals.length > 0) {
        ...
        if (sharedUserHospitals.length > 0) {
          const { data: sharedData, error: sharedErr } = await fetchAllRows(() => supabase.from("sessions")
            .select("*")
            .in("hospital", sharedUserHospitals)
            .order("created_at", { ascending: true }));
          if (sharedErr) setDbError("Some session data failed to load — totals may be incomplete. Reload to retry.");
      ...
      if (myProfileRow?.role === "clinical" && myProfileRow?.clinical_hospital) {
        const { data: clinicalData, error: clinicalErr } = await fetchAllRows(() => supabase.from("sessions")
          .select("*")
          .eq("hospital", myProfileRow.clinical_hospital)
          .order("created_at", { ascending: true }));
        if (clinicalErr) setDbError("Some session data failed to load — totals may be incomplete. Reload to retry.");
      ...
      const { data: allData, error: allErr } = await fetchAllRows(() => supabase.from("sessions")
        ...
        .order("created_at", { ascending: true }));
      if (allErr) setDbError("Some session data failed to load — totals may be incomplete. Reload to retry.");
      setAllEntries(allData || []);

      // Admins get all sessions + audit log + user profiles
      if (isAdminUser) {
        const { data: adminData, error: adminErr } = await fetchAllRows(() => supabase.from("sessions").select("*").order("created_at", { ascending: true }));
        if (adminErr) setDbError("Some session data failed to load — totals may be incomplete. Reload to retry.");
        ...
        const { data: inviteData } = await supabase.from("clinical_invites").select("*").order("created_at", { ascending: false });
        setClinicalInvites(inviteData || []);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data, error: regionErr } = await fetchAllRows(() => supabase.from("sessions").select("*")
        .in("logged_by", repNames)
        .order("created_at", { ascending: true }));
      if (regionErr) setDbError("Some session data failed to load — totals may be incomplete. Reload to retry.");
  ...
  useEffect(() => {
    ...
    (async () => {
      const { data, error: kamErr } = await fetchAllRows(() => supabase.from("sessions").select("*")
        .in("hospital", kamAccounts)
        .order("created_at", { ascending: true }));
      if (kamErr) setDbError("Some session data failed to load — totals may be incomplete. Reload to retry.");
      setKamEntries(data || []);
    })();
  ...
  const handleTouchEnd = async (e) => {
    ...
    if (dy > 80 && dx < 40 && scrollTop < 10 && (tab === "dashboard" || tab === "history")) {
      ...
      const { data, error: refreshErr } = await fetchAllRows(() => supabase.from("sessions").select("*").eq("logged_by", uName).order("created_at", { ascending: true }));
      // On a partial fetch, keep the existing full dataset rather than replacing it with a truncated one.
      if (refreshErr) setDbError("Refresh failed partway — showing previously loaded data. Pull again or reload to retry.");
    ...
    <div style={{ background: C.bg, minHeight: "100vh", color: C.ink, fontFamily: "'IBM Plex Sans', sans-serif" }}>
      ...
      <div className="header-wrap" style={{ background: C.surface, borderBottom: `1px solid ${C.border}`, boxShadow: "0 1px 4px rgba(79,110,119,0.06)" }}>
        <div style={{ maxWidth: 1120, margin: "0 auto", padding: "0 32px" }} className="header-outer">
          ...
          <div style={{ display: "flex", marginTop: 4 }} className="nav-tabs">
            ...
            {!isClinical && <Tab id="assessment" label="ASSESSMENT" />}
        ...
        {tab === "admin" && isAdmin && (
          <div>
            ...
            <div style={{ display: "flex", borderBottom: `1px solid ${C.border}`, marginBottom: 24, gap: 0, overflowX: "auto", WebkitOverflowScrolling: "touch" }}>
              {[["sessions","ALL SESSIONS"],["deletions","DELETION REQUESTS"],["users","USER MANAGEMENT"],["invites","CLINICAL INVITES"],["hospitals","HOSPITALS"],["auto_reports","AUTO REPORTS"],["audit","AUDIT LOG"]].map(([id, label]) => {
            ...
            {/* ── CLINICAL INVITES SECTION ── */}
            {adminSection === "invites" && (
              <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                {/* Create invite form */}
                <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, padding: "24px" }}>
                  <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 10, color: C.inkLight, letterSpacing: "0.1em", marginBottom: 4 }}>SEND CLINICAL USER INVITE</div>
                  <div style={{ fontSize: 13, color: C.inkMid, marginBottom: 18 }}>
                    Invite a hospital clinical contact to view compliance reporting for sessions logged by a specific rep at their facility.
                  </div>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 12 }} className="invite-form-grid">
                    ...
                    <div>
                      ...
                      <select value={inviteForm.rep_id}
                        onChange={e => setInviteForm(f => ({ ...f, rep_id: e.target.value }))}
                        style={{ width: "100%", padding: "10px 12px", border: `1px solid ${C.border}`, borderRadius: 8, fontSize: 13, background: C.bg, color: C.ink, fontFamily: "inherit" }}>
                    ...
                    <div>
                      <label style={{ display: "block", fontSize: 11, color: C.inkLight, marginBottom: 6, fontFamily: "'IBM Plex Mono', monospace", letterSpacing: "0.05em" }}>CLINICAL USER NAME</label>
                      <input type="text" value={inviteForm.name}
                        onChange={e => setInviteForm(f => ({ ...f, name: e.target.value }))}
                        placeholder="e.g. Jane Smith"
                        style={{ width: "100%", padding: "10px 12px", border: `1px solid ${C.border}`, borderRadius: 8, fontSize: 13, background: C.bg, color: C.ink, fontFamily: "inherit", boxSizing: "border-box" }} />
                    </div>
                    <div>
                      <label style={{ display: "block", fontSize: 11, color: C.inkLight, marginBottom: 6, fontFamily: "'IBM Plex Mono', monospace", letterSpacing: "0.05em" }}>EMAIL</label>
                  ...
                  {inviteError && (
                    <div style={{ background: C.redLight, color: C.red, padding: "10px 14px", borderRadius: 8, fontSize: 12, marginBottom: 12, border: `1px solid ${C.red}33` }}>
                      {inviteError}
                    </div>
                  )}
                  {inviteSuccess && (
                    <div style={{ background: C.greenLight, color: C.green, padding: "10px 14px", borderRadius: 8, fontSize: 12, marginBottom: 12, border: `1px solid ${C.green}33` }}>
                      {inviteSuccess}
                    </div>
                  )}

                  <button onClick={handleCreateInvite} disabled={sendingInvite}
                    style={{ background: C.primary, color: "white", border: "none", borderRadius: 8, padding: "10px 22px", fontSize: 11, fontFamily: "'IBM Plex Mono', monospace", letterSpacing: "0.06em", cursor: sendingInvite ? "wait" : "pointer", opacity: sendingInvite ? 0.6 : 1 }}>
                ...
                <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, padding: "24px" }}>
                  <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 10, color: C.inkLight, letterSpacing: "0.1em", marginBottom: 16 }}>
                    INVITES ({clinicalInvites.length})
                  </div>
                  {clinicalInvites.length === 0 ? (
                    <div style={{ fontSize: 13, color: C.inkLight, padding: "20px 0" }}>No invites sent yet.</div>
                  ) : (
                    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                      {clinicalInvites.map(inv => {
                        const status = inviteStatus(inv);
                        const rep = userProfiles.find(p => p.id === inv.rep_id);
                        const repLabel = rep ? (rep.full_name || rep.email) : "(unknown rep)";
                        const statusColors = {
                          pending: { bg: C.primaryLight, fg: C.primary, label: "PENDING" },
                          used:    { bg: C.greenLight, fg: C.green, label: "ACCEPTED" },
                          expired: { bg: C.redLight, fg: C.red, label: "EXPIRED" },
                        }[status];
                        const expiresOrUsed = status === "used"
                          ? `Accepted ${new Date(inv.used_at).toLocaleDateString()}`
                          : status === "expired"
                            ? `Expired ${new Date(inv.expires_at).toLocaleDateString()}`
                            : `Expires ${new Date(inv.expires_at).toLocaleDateString()}`;
                        return (
                          <div key={inv.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 14px", background: C.bg, borderRadius: 8, flexWrap: "wrap" }}>
                            <div style={{ flex: 1, minWidth: 200 }}>
                              <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                                <div style={{ fontSize: 13, fontWeight: 500, color: C.ink }}>{inv.name}</div>
                                <span style={{ fontSize: 9, background: statusColors.bg, color: statusColors.fg, border: `1px solid ${statusColors.fg}33`, borderRadius: 10, padding: "1px 8px", fontFamily: "'IBM Plex Mono', monospace", letterSpacing: "0.05em" }}>
                                  {statusColors.label}
                                </span>
                              </div>
                              <div style={{ fontSize: 11, color: C.inkLight, marginTop: 3 }}>
                                {inv.email} · {inv.hospital} · Rep: {repLabel}
                              </div>
                            ...
                            {status === "pending" && (
                              <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
                                <button onClick={() => handleCopyInviteLink(inv)}
                                  style={{ background: "none", border: `1px solid ${C.border}`, borderRadius: 6, padding: "5px 10px", fontSize: 10, fontFamily: "'IBM Plex Mono', monospace", color: C.inkMid, cursor: "pointer", letterSpacing: "0.05em" }}>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }} onClick={() => setShowChangelog(false)}>
          <div style={{ background: C.surface, borderRadius: 16, maxWidth: 540, width: "100%", padding: "36px 40px", boxShadow: "0 20px 60px rgba(0,0,0,0.3)", maxHeight: "80vh", overflowY: "auto" }} onClick={e => e.stopPropagation()}>
            ...
            {[
              ...
              { version: "3.6", date: "July 2026", badge: null, items: [
                "Range-aware period comparison in PDF and PPTX exports",
              ]},
              { version: "3.5", date: "June 2026", badge: null, items: [
      ...
      <div className="bottom-nav" style={{
        ...
        {[
          ...
          ...(!isClinical ? [{ id: "assessment", label: "Assess", icon: (active) => (
            <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={active ? C.primary : C.inkLight} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
