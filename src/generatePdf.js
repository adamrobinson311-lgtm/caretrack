...
export async function generatePdf(entries, summary = "", returnBase64 = false, hospitalFilter = "", preparedBy = "", branding = null, chartData = [], mom = null) {
  ...
  if (mom?.hasData) {
    doc.addPage();
    const momPageNum = 3;
    addHeader(doc, momPageNum, totalPages, preparedBy, brandHeader);
    ...
    // Summary cards row
    const cardDefs = [
      { label: mom.thisMonth, value: mom.thisAvg !== null ? `${mom.thisAvg}%` : "—", sub: `${mom.thisSessions} session${mom.thisSessions !== 1 ? "s" : ""}`, color: mom.thisAvg !== null ? pctColor(mom.thisAvg) : BRAND.inkLight },
      { label: mom.lastMonth, value: mom.lastAvg !== null ? `${mom.lastAvg}%` : "—", sub: `${mom.lastSessions} session${mom.lastSessions !== 1 ? "s" : ""}`, color: mom.lastAvg !== null ? pctColor(mom.lastAvg) : BRAND.inkLight },
  ...
  doc.addPage();
  const histPageNum = mom?.hasData ? 4 : 3;
